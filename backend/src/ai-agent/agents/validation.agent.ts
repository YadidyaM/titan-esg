...
export class ValidationAgent {
  ...
  // Expected data ranges and patterns
  ...
  private async performAIValidation(data: any): Promise<{ isValid: boolean; errors: string[]; warnings: string[] }> {
    try {
  ...
  private parseAIValidationResult(content: string): { isValid: boolean; errors: string[]; warnings: string[] } {
    try {
  ...
  private performRuleBasedValidation(data: any): { isValid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    ...
    Object.entries(rules).forEach(([field, rule]) => {
      ...
      if (typeof value === 'number') {
        if (typeof rule === 'object' && rule !== null && 'min' in rule && 'max' in rule) {
          const min = (rule as any).min;
          const max = (rule as any).max;
  ...
  private validateGeneralData(data: any, errors: string[], warnings: string[]) {
  ...
  private detectAnomalies(data: any): DataAnomaly[] {
    const anomalies: DataAnomaly[] = [];

    try {
      // Statistical anomaly detection for numerical values
  ...
  private extractNumericalFields(obj: any, path: string = ''): Array<{ path: string; value: number }> {
    const fields: Array<{ path: string; value: number }> = [];
  ...
  private detectStatisticalAnomaly(path: string, value: number, data: any): DataAnomaly | null {
    // Simple outlier detection using z-score approximation
    const similarValues = this.findSimilarValues(path, data);
    
    if (similarValues.length < 3) return null;
  ...
  private findSimilarValues(path: string, data: any): number[] {
    // Find values from similar fields or historical data
    const values: number[] = [];
    
    // Extract base field name (e.g., "emissions" from "environmental.emissions")
    const baseField = path.split('.').pop();
    
    if (baseField) {
      // Look for the same field in different categories
  ...
  private detectPatternAnomalies(data: any): DataAnomaly[] {
    const anomalies: DataAnomaly[] = [];
  ...
  private calculateDataQuality(data: any): { completeness: number; accuracy: number; consistency: number; timeliness: number } {
    ...
    // Simplified scoring - in practice, these would be more sophisticated
  ...
  private countPresentFields(obj: any): number {
    let count = 0;
    Object.values(obj).forEach(value => {
      if (value !== undefined && value !== null) {
  ...
  private countDataErrors(data: any): number {
    // Simplified error counting
    let errors = 0;
    
    if (data.environmental?.emissions < 0) errors++;
    if (data.environmental?.renewableEnergy > 100) errors++;
    if (data.social?.employeeSatisfaction > 100) errors++;
    
    return errors;
  }

  private countInconsistencies(data: any): number {
    // Simplified inconsistency counting
    let inconsistencies = 0;
    
    if (data.environmental?.emissions === 0 && data.environmental?.energy > 1000) inconsistencies++;
    if (data.social?.employeeCount > 0 && data.social?.employeeSatisfaction === undefined) inconsistencies++;
    
    return inconsistencies;
  }

  private calculateTimeliness(lastUpdated: any): number {
    try {
  ...
  private fallbackDataValidation(data: any): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
